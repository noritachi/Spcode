...
<html>
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" style="height: 80px;">
            <div class="container">
                <div class="navbar-header">
                    ...
                    <h1 style="color: white;margin-top: 10px;" id="myhead"><a
                            style="color: white;text-decoration: null;" href="/spcode">SPCODE</a></h1>
                </div>
                <div class="collapse navbar-collapse" id="navi">
                    ...
                    <ul class="nav navbar-nav navbar-right">
                        ...
                        <li> <a href="../../logout.php" id="our-location" class="btn-success"><?php
        ...
        <div class="sidemenu" id="sidebarleftmenu">
            ...
            ?>
                ...
                <form action="fetch_main_content.php" method="POST">
                    ...
                    <input type="hidden" name="txt2" value="<?php echo $i ?>">
                    <button style="background-color: transparent;border: none;text-align:left;color: white;">
                        <li style="width: 300px;" class="item_programming"><?php echo $res['topic_name']; ?></li>
                    </button>
 ...
 -->
        ...
        const activePlus = Number(active) + 1;
        const activeRev = Number(active) - 1;
        const course = urlParams.get('course_name')
        var li = document.getElementsByTagName('li')[0].style = "color:red";
        let idNext = document.getElementById('btn_next');
        let idRev = document.getElementById('btn_rev');
        let listOption = document.getElementsByClassName('item_programming');

        listOption[active].classList.add('active');

        /// 

        if (active < 1) {
            idRev.classList.add("disable_data");
        } else {
            idRev.classList.remove("disable_data");
        }

        if (active >= 13) {
            idNext.classList.add("disable_data");
        } else {
            idNext.classList.remove("disable_data");
        }

        idNext.addEventListener('click', () => {
            window.location.href = window.origin + window.location.pathname + '?course_name=' + course +
                '&active=' +
                activePlus
        })


        idRev.addEventListener('click', () => {
            window.location.href = window.origin + window.location.pathname + '?course_name=' + course +
                '&active=' +
                activeRev
        })
